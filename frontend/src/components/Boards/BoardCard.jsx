...
function BoardCard({ board }) {
  const { name, updatedAt, color } = board;
  ...
  return (
    <button className={styles.boardCard}>
      <div className={styles.boardCardPreview} style={{ backgroundColor: color }}>
        <div className={styles.boardCardOverlay}>
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
      ...
      <div className={styles.boardCardInfo}>
        <h3 className={styles.boardCardName}>{name}</h3>
        <p className={styles.boardCardDate}>Updated {formatDate(updatedAt)}</p>
      </div>
    </button>
  );
}
